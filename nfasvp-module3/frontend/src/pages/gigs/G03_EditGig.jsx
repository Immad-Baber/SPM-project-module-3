...
export default function EditGig({ onNavigate, params, role }) {
  const { gig, loading: loadingGig, error: errorGig } = useGig(params?.id);
  const { updateGig, loading: saving, error: errorSave } = useUpdateGig();
  ...
  useEffect(() => {
    if (gig) {
      setForm({
        ...
        required_skills: Array.isArray(gig.required_skills) 
          ? gig.required_skills.map(s => typeof s === 'string' ? s : (s.tag?.name || s.name)).join(", ") 
          : "",
        pricing: gig.pricing_tiers?.length === 3 ? gig.pricing_tiers : [
          { name: "Basic", price: "", delivery_days: "", desc: "" },
          { name: "Standard", price: "", delivery_days: "", desc: "" },
          { name: "Premium", price: "", delivery_days: "", desc: "" },
        ]
  ...
  const handleSubmit = async (e) => {
    ...
    const success = await updateGig(gig.id, payload);
    if (success) {
  ...
  if (loadingGig) return <div style={{ padding: 60, textAlign: "center" }}>Loading gig data...</div>;
  if (!gig && !loadingGig) return <div style={{ padding: 60, textAlign: "center" }}>Gig not found.</div>;
    ...
    <div style={{ display: "flex", flexDirection: "column", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif", background: C.bgPage }}>
      ...
      <main style={{ flex: 1, padding: "40px 24px" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", background: C.white, border: `1px solid ${C.border}`, borderRadius: 10, padding: 32, boxShadow: "0 4px 20px rgba(0,0,0,0.05)" }}>
          ...
          <h1 style={{ margin: "0 0 8px", fontSize: 28, fontWeight: 800, color: C.black }}>Edit Gig</h1>
          <p style={{ margin: "0 0 32px", color: C.textSecondary }}>Update your service details and pricing packages.</p>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <section style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
                {form.pricing.map((tier, i) => (
                  <div key={tier.name || i} style={{ background: "#F8FAFC", border: `1px solid ${C.border}`, borderRadius: 8, padding: 16, display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {errorSave && <div style={{ color: "red", fontSize: 14, textAlign: "right" }}>{errorSave}</div>}
          </form>

          <div style={{ marginTop: 32 }}>
            <StickyNote text="🔗 Save Changes → API call to update gig · Redirects to G03_MyGigs on success." />
          </div>
        </div>
      </main>
    </div>
